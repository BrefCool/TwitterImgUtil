...
def draw_text_on_images(file_path, text, save_dir):
    if not os.path.exists(save_dir):
        print("cannot find path:%s" % save_dir)
        return
    try:
...
def draw_labels_on_images(image_client, image_dir, save_dir):
    ...
    for file_name in images_list:
        ...
        draw_text_on_images(os.path.join(image_dir, file_name), labels_str, save_dir)
